...
refiner_pipeline = get_refiner_pipeline()
upscaler_pipeline = get_upscaler_pipeline()

def get_base_image(prompt, negative_prompt, width, height, scale, steps, seed):
...
def imagine(prompt, negative_prompt, width, height, scale, steps, seed, upscaler):
    ...
    images.append(refiner_image)

    if upscaler == True:
        upscaler_image = upscaler_pipeline(
            prompt=prompt,
            negative_prompt=negative_prompt,
            image=refiner_image,
...
gr.Interface(
    ...
    inputs=[
        ...
        gr.Checkbox(label="upscale"),
    ],
    outputs=gr.Gallery(label="Generated images (Origin / Refiner / Upscale)", show_label=True).style(columns=[2], rows=[2], object_fit="contain", height="auto"),
